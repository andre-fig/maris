...
export class ModelEncMetadata2026091800000 implements MigrationInterface {
  ...
  async up(runner: QueryRunner): Promise<void> {
    await runner.createTable(new Table({
      ...
      foreignKeys: [{ columnNames: ['version_id'], referencedTableName: 'chart_versions', referencedColumnNames: ['id'], onDelete: 'CASCADE' }],
      indices: [{ name: 'chart_cells_version_name_unique', columnNames: ['version_id', 'name'], isUnique: true }],
    }));
    await runner.createTable(new Table({
      ...
      foreignKeys: [{ columnNames: ['cell_id'], referencedTableName: 'chart_cells', referencedColumnNames: ['id'], onDelete: 'CASCADE' }],
      indices: [{ name: 'chart_coverages_cell_index', columnNames: ['cell_id'] }],
    }));
    await runner.createTable(new Table({
      name: 'chart_surveys',
      columns: [
      ...
      foreignKeys: [{ columnNames: ['cell_id'], referencedTableName: 'chart_cells', referencedColumnNames: ['id'], onDelete: 'CASCADE' }],
      indices: [{ name: 'chart_surveys_cell_index', columnNames: ['cell_id'] }],
    }));
    ...
    const versions = await runner.query('SELECT id, edition_metadata FROM chart_versions');
    ...
    for (const version of versions) {
      for (const cell of version.edition_metadata ?? []) {
        ...
        await runner.query(`INSERT INTO chart_cells
        ...
        [id, version.id, cell.name, m.source ?? null, m.agencyCode ?? null, cell.edition,
          cell.updateNumber, cell.updatesApplied ?? [], m.issueDate ?? null,
          m.updateApplicationDate ?? null, m.compilationScale ?? null, m.horizontalDatum ?? null,
          m.verticalDatum ?? null, m.soundingDatum ?? null, m.coveredAreaNames ?? []]);
        for (const feature of m.coverage ?? []) {
          await runner.query('INSERT INTO chart_coverages (id,cell_id,category,geometry) VALUES ($1,$2,$3,$4)',
            [randomUUID(), id, feature.properties.CATCOV ?? null, JSON.stringify(feature.geometry)]);
        }
        for (const [objectClass, features] of Object.entries(m.metaObjects ?? {})) {
          for (const feature of features as Array<{ properties: Record<string, unknown>; geometry: unknown }>) {
            const p = feature.properties;
            if (!['CATZOC', 'SORIND', 'SORDAT', 'SURSTA', 'SUREND'].some((key) => p[key] != null)) continue;
            await runner.query(`INSERT INTO chart_surveys
              (id,cell_id,object_class,data_quality,survey_source,survey_date,survey_started_at,survey_ended_at,geometry)
              VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)`,
            [randomUUID(), id, objectClass, p.CATZOC ?? null, p.SORIND ?? null, date(p.SORDAT),
              date(p.SURSTA), date(p.SUREND), JSON.stringify(feature.geometry)]);
          }
        }
      }
    }
