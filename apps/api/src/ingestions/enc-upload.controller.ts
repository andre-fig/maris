...
export class EncUploadController {
  ...
  @Post("multipart")
  async create(
    @Body() body: { filename: string; size?: number; checksumSha256?: string },
  ) {
    ...
    await this.db
      .getRepository(EncUpload)
      .save({
        id: randomUUID(),
        uploadId: upload.uploadId,
        objectKey: key,
        sourceFilename: body.filename,
        expectedSize: body.size == null ? null : String(body.size),
  ...
  async fromUrl(@Body() body: { url: string; filename?: string }) {
    ...
    const upload = await this.db
      ...
      .save({
        id: randomUUID(),
        uploadId: `url-${randomUUID()}`,
        objectKey: key,
        sourceFilename:
          body.filename ?? source.pathname.split("/").pop() ?? "source.zip",
        expectedSize: null,
    ...
    await this.db
      .getRepository(EncUpload)
      .update(upload.id, { ingestionId, status: "queued" });
    return {
      uploadId: upload.uploadId,
      objectKey: key,
      ingestionId,
  ...
  @Get(":uploadId") async progress(@Param("uploadId") uploadId: string) {
    return this.db.getRepository(EncUpload).findOneByOrFail({ uploadId });
  }
  @Post(":uploadId/retry")
  async retry(@Param("uploadId") uploadId: string) {
    ...
    if (!(await this.storage.tryHead(upload.objectKey)))
      throw new BadRequestException(
        "Source object is not available in object storage",
      );
    const ingestionId = randomUUID();
    await this.dispatcher.dispatch({
      archivePath: "",
      ...
      objectKey: upload.objectKey,
      sourceFilename: upload.sourceFilename,
      ...(upload.checksumSha256 ? { checksum: upload.checksumSha256 } : {}),
      ...(upload.expectedSize
        ? { sizeBytes: Number(upload.expectedSize) }
        : {}),
    });
  ...
  async complete(
    @Param("uploadId") uploadId: string,
    @Body()
    body: { parts: { partNumber: number; etag: string; size?: number }[] },
  ) {
    const repo = this.db.getRepository(EncUpload);
    const upload = await repo.findOneByOrFail({ uploadId });
    if (upload.status === "queued" && upload.ingestionId)
      return {
        uploadId,
        objectKey: upload.objectKey,
        status: "queued",
    ...
    if (!body.parts?.length)
      throw new BadRequestException("At least one uploaded part is required");
    ...
    if (!existingObject) {
      try {
        ...
        if (!(await this.storage.tryHead(upload.objectKey))) throw error;
      }
    }
    upload.parts = body.parts;
  ...
  async abort(@Param("uploadId") uploadId: string) {
    const repo = this.db.getRepository(EncUpload);
    const upload = await repo.findOneByOrFail({ uploadId });
    await this.storage.abortMultipart(upload.objectKey, uploadId);
    upload.status = "aborted";
    await repo.save(upload);
    return { uploadId, status: upload.status };
  }
}
