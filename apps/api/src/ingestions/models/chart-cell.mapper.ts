...
export function mapChartCell(versionId: string, input: ProcessedCell): ChartCell {
  ...
  cell.coverages = (metadata?.coverage ?? []).map((feature) => Object.assign(new ChartCoverage(), {
    id: randomUUID(), cellId: cell.id,
    category: typeof feature.properties.CATCOV === 'number' ? feature.properties.CATCOV : null,
    geometry: feature.geometry,
  }));
  cell.surveys = Object.entries(metadata?.metaObjects ?? {}).flatMap(([objectClass, features]) =>
    features.filter(({ properties: p }) =>
      ['CATZOC', 'SORIND', 'SORDAT', 'SURSTA', 'SUREND'].some((key) => p[key] != null),
    ).map((feature) => {
      const p = feature.properties;
      return Object.assign(new ChartSurvey(), {
        id: randomUUID(), cellId: cell.id, objectClass,
        ...
        surveyStartedAt: encDate(p.SURSTA), surveyEndedAt: encDate(p.SUREND),
        geometry: feature.geometry,
      });
    }),
  );
