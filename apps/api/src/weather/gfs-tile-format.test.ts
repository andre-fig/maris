...
test('measures the 10 degree tile representations at GFS 0.25 degrees', () => {
  const count = 41 * 41;
  const values = (scale: number) => Array.from({ length: count }, (_, index) => index % 17 === 0 ? null : scale + index / 100);
  const grid = {
    model: 'gfs' as const,
    ...
    fields: {
      windU: values(1), windV: values(2), temperature: values(273),
      precipitationRate: values(0.001), cloudCover: values(50), pressure: values(101_000),
      gust: values(4), humidity: values(70),
    },
  };
