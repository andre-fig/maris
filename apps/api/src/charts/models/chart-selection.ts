import type { EncMetadataFeature, ProcessedCell } from '../../ingestions/models/processing.js';
...
export type CoverageCell = {
  ...
  coverages: { category: number | null; geometry: EncMetadataFeature['geometry'] }[];
};
...
function prepareRing(input: unknown): Ring | null {
  ...
  for (const p of input) {
    ...
    if (previous) longitude += 360 * Math.round((previous[0] - longitude) / 360);
...
function prepareGeometry(geometry: EncMetadataFeature['geometry']): Polygon[] {
  if (!geometry) return [];
  const input = geometry.type === 'Polygon' ? [geometry.coordinates] : geometry.type === 'MultiPolygon' ? geometry.coordinates : [];
...
function inRing(point: Position, ring: Ring): number {
  ...
  const x = point[0] + 360 * Math.round(((w + e) / 2 - point[0]) / 360), y = point[1];
  if (x < w || x > e || y < s || y > n) return 0;
  let inside = false;
  for (let i = 0, j = ring.points.length - 1; i < ring.points.length; j = i++) {
    const [ax, ay] = ring.points[j]!, [bx, by] = ring.points[i]!;
    const cross = (x - ax) * (by - ay) - (y - ay) * (bx - ax);
    if (Math.abs(cross) <= 1e-12 && x >= Math.min(ax, bx) && x <= Math.max(ax, bx) && y >= Math.min(ay, by) && y <= Math.max(ay, by)) return -1;
...
function inPolygons(point: Position, polygons: Polygon[]): boolean {
  return polygons.some(([outer, ...holes]) => outer && inRing(point, outer) !== 0 && holes.every((hole) => inRing(point, hole) === 0));
}

export function coversPoint(geometry: EncMetadataFeature['geometry'], point: Position): boolean {
  return inPolygons(point, prepareGeometry(geometry));
}

function newest(a: CoverageCell, b: CoverageCell) {
  return Number(b.edition ?? 0) - Number(a.edition ?? 0) || b.updateNumber - a.updateNumber ||
    (b.updateApplicationDate ?? '').localeCompare(a.updateApplicationDate ?? '') ||
...
export class ChartSelection {
  ...
  constructor(input: CoverageCell[]) {
    ...
    const scale = (c: CoverageCell) => c.compilationScale && c.compilationScale > 0 ? c.compilationScale : Infinity;
    this.cells = [...latest.values()].sort((a, b) => scale(a) - scale(b) ||
      (b.updateApplicationDate ?? b.issueDate ?? '').localeCompare(a.updateApplicationDate ?? a.issueDate ?? '') ||
      a.name.localeCompare(b.name)).map((cell) => ({
      cell,
      included: cell.coverages.filter((c) => c.category === 1).flatMap((c) => prepareGeometry(c.geometry)),
      excluded: cell.coverages.filter((c) => c.category === 2).flatMap((c) => prepareGeometry(c.geometry)),
    }));
  }
  at(point: Position): CoverageCell | null {
    return this.cells.find((c) => inPolygons(point, c.included) && !inPolygons(point, c.excluded))?.cell ?? null;
  }
}

export function coverageCells(cells: ProcessedCell[]): CoverageCell[] {
  return cells.map((c) => ({
    name: c.name, edition: c.edition, updateNumber: c.updateNumber,
    ...
    coverages: (c.metadata?.coverage ?? []).map((f) => ({
      category: typeof f.properties.CATCOV === 'number' ? f.properties.CATCOV : null,
      geometry: f.geometry,
    })),
  }));
}
