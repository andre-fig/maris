...
import { ChartSelection, coversPoint, type CoverageCell } from './chart-selection.js';

const square = (w: number, s: number, e: number, n: number) => ({ type: 'Polygon', coordinates: [[[w,s],[e,s],[e,n],[w,n],[w,s]]] });
const cell = (name: string, scale: number, overrides: Partial<CoverageCell> = {}): CoverageCell => ({
  name, compilationScale: scale, edition: '1', updateNumber: 0, issueDate: '2020-01-01', updateApplicationDate: '2020-01-01',
  coverages: [{ category: 1, geometry: square(-81, 25, -79, 27) }], ...overrides,
});
...
test('latest edition/updates of a cell precede detail selection between different cells', () => {
  const selection = new ChartSelection([
    ...
    cell('coastal', 80000, { issueDate: '2026-01-01', updateApplicationDate: '2026-01-01' }),
  ]);
...
test('uses CATCOV=1 and excludes holes/CATCOV=2; coarse chart fills uncovered areas', () => {
  const harbor = cell('harbor', 1000, { coverages: [
...
test('handles multipolygons and antimeridian without claiming the rest of the world', () => {
  const shape = { type: 'MultiPolygon', coordinates: [square(179,-1,-179,1).coordinates, square(10,10,11,11).coordinates] };
...
test('same-scale ties use update date then cell name, independent of input order', () => {
  const a = cell('A',22000), b = cell('B',22000);
