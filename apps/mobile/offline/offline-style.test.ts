...
import { validateArea, withOfflineSoundings, type ChartSnapshot, type StyleSnapshot } from './offline-style';

const chart: ChartSnapshot = {version:'v1', tiles:['https://example.test/tiles/v1/{z}/{x}/{y}.pbf'], bounds:[-81,25,-80,26], minzoom:8, maxzoom:16};
...
test('rejects mutable chart urls and invalid download bounds', () => {
  assert.throws(() => withOfflineSoundings(style,{...chart, tiles:['https://example.test/latest/{z}/{x}/{y}.pbf']}));
