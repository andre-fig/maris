...
test("heading prefers reliable true heading and falls back to magnetic heading", () => {
  assert.equal(resolveHeading(82, 25, 1), 82);
  assert.equal(resolveHeading(-1, 25, 1), 25);
  assert.equal(resolveHeading(null, 25, 1), 25);
  assert.equal(resolveHeading(370, 25, 1), 10);
  assert.equal(resolveHeading(82, 25, 0), null);
  assert.equal(resolveHeading(null, null, 1), null);
});
