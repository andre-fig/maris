...
test("repeated native camera events settle without a render/command feedback loop", async () => {
  ...
  try {
    const result = await build({ entryPoints: [path.join(__dirname, "../App.tsx")],
      ...
      plugins: [{ name: "native-feedback", setup(builder) {
        ...
        builder.onLoad({ filter: /.*/, namespace: "mock" }, ({ path: name }) => {
          ...
          else if (name.includes("/weather/")) contents = 'export const useCurrentViewportWeather=()=>({onTouchStart(){},onTouchEnd(){},onCameraChanging(){},onCameraDidChange(){}});';
          ...
          else if (name.endsWith("NavigationDataPanel")) contents = 'export const NavigationDataPanel="NavigationDataPanel",WeatherConditionsPanel="WeatherConditionsPanel";';
    ...
    assert.equal(map().props.mapStyle, "https://tiles.openfreemap.org/styles/bright");
    await act(async () => controls().props.onMapModeChange("liberty"));
    assert.equal(map().props.mapStyle, "https://tiles.openfreemap.org/styles/liberty");
