...
// Exactly one last complete field, stored outside disposable HTTP caches.
...
public:
  ...
  Snapshot load(const Plan &expected) const {
    ...
    if (!in || hash != checksum(*snapshot.field, snapshot.catalog) || in.peek() != std::char_traits<char>::eof()) return {};
